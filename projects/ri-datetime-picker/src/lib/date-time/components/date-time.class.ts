...
@Directive()
export abstract class RIDateTime<T> {
    /**
    ...
    set firstDayOfWeek(value: number) {
        ...
        if (value > 6 || value < 0) {
            this._firstDayOfWeek = undefined as any;
        } else {
    ...
    protected constructor(
        ...
        protected dateTimeFormats: RIDateTimeFormats
    ) {
        if (!this.dateTimeAdapter) {
            throw Error(
                `RIDateTimePicker: No provider found for DateTimeAdapter. You must import one of the following ` +
        ...
        this._id = `ri-dt-picker-${nextUniqueId++}`;
    }
