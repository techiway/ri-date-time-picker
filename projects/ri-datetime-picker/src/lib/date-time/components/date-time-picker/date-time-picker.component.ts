import { ScrollStrategy, Overlay, BlockScrollStrategy, OverlayConfig, OverlayRef, PositionStrategy } from '@angular/cdk/overlay';
import { ChangeDetectionStrategy, ChangeDetectorRef, Component, ComponentRef, EventEmitter, Inject, InjectionToken, Input, NgZone, OnDestroy, OnInit, Optional, Output, ViewContainerRef, inject } from '@angular/core';
import { PickerMode, PickerType, RIDateTime, SelectMode } from '../date-time.class';
import { coerceBooleanProperty, coerceArray } from '@angular/cdk/coercion';
import { ESCAPE, UP_ARROW } from '@angular/cdk/keycodes';
import { ComponentPortal } from '@angular/cdk/portal';
import { DOCUMENT } from '@angular/common';
import { Subscription, take, merge, filter } from 'rxjs';
...
import { RIDialogRef } from '../../../dialog/dialog-ref.class';
import { RIDialogService } from '../../../dialog/dialog.service';


/** Injection token that determines the scroll handling while the dtPicker is open. */
export const RI_DTPICKER_SCROLL_STRATEGY = new InjectionToken<
    () => ScrollStrategy
>('ri-dtpicker-scroll-strategy');

/** @docs-private */
export function RI_DTPICKER_SCROLL_STRATEGY_PROVIDER_FACTORY(
    overlay: Overlay
): () => BlockScrollStrategy {
...
/** @docs-private */
export const RI_DTPICKER_SCROLL_STRATEGY_PROVIDER = {
    provide: RI_DTPICKER_SCROLL_STRATEGY,
    deps: [Overlay],
    useFactory: RI_DTPICKER_SCROLL_STRATEGY_PROVIDER_FACTORY
};


@Component({
  selector: 'ri-date-time',
  exportAs: 'riDateTime',
...
export class RIDateTimeComponent<T> extends RIDateTime<T> implements OnInit, OnDestroy {
   ...
   private pickerContainerPortal: ComponentPortal<
       RIDateTimeContainerComponent<T>
   >;
   private pickerContainer: RIDateTimeContainerComponent<T>;
   private popupRef: OverlayRef;
   private dialogRef: RIDialogRef<RIDateTimeContainerComponent<T>>;
   ...
   private _dtInput: RIDateTimeInputDirective<T>;
   get dtInput() {
   ...
   private dialogService = inject(RIDialogService);
   
   constructor(
       public overlay: Overlay,
       private viewContainerRef: ViewContainerRef,
    //    private dialogService: RIDialogService,
       private ngZone: NgZone,
       protected changeDetector: ChangeDetectorRef,
       @Optional() protected override dateTimeAdapter: DateTimeAdapter<T>,
       @Inject(RI_DTPICKER_SCROLL_STRATEGY) defaultScrollStrategy: any,
       @Optional()
       @Inject(RI_DATE_TIME_FORMATS)
       protected override dateTimeFormats: RIDateTimeFormats,
       @Optional()
       @Inject(DOCUMENT)
   ...
   public registerInput(input: RIDateTimeInputDirective<T>): void {
       if (this._dtInput) {
           throw Error(
               'A RI DateTimePicker can only be associated with a single input.'
           );
       }
   ...
   private openAsDialog(): void {
       this.dialogRef = this.dialogService.open(
           RIDateTimeContainerComponent,
           {
               ...
               paneClass: ['ri-dt-dialog', ...coerceArray(this.panelClass)],
   ...
   private openAsPopup(): void {
       if (!this.pickerContainerPortal) {
           this.pickerContainerPortal = new ComponentPortal<
               RIDateTimeContainerComponent<T>
           >(RIDateTimeContainerComponent, this.viewContainerRef);
       }
   ...
   private createPopup(): void {
       const overlayConfig = new OverlayConfig({
           ...
           panelClass: ['ri-dt-popup', ...coerceArray(this.panelClass)]
       });
   ...
   private createPopupPositionStrategy(): PositionStrategy {
       return this.overlay
           ...
           .withTransformOriginOn('.ri-dt-container')
