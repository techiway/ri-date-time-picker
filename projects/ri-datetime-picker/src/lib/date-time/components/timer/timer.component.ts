...
@Component({
  selector: 'ri-date-time-timer',
  templateUrl: './timer.component.html',
  styleUrls: ['./timer.component.scss'],
  preserveWhitespaces: false,
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: {
    // '[class.ri-dt-timer]': 'riDTTimerClass',
    '[attr.tabindex]': 'riDTTimeTabIndex'
}
})
export class RITimerComponent<T> implements OnInit {
   ...
   get riDTTimerClass(): boolean {
       return true;
   }

   get riDTTimeTabIndex(): number {
       return -1;
   }
   ...
   public setTimeValue(value: any): void {
        ...
        const m = this.dateTimeAdapter.setHours(this.pickerMoment, hours);
        const m1 = this.dateTimeAdapter.setMinutes(m, minutes);

        console.log(m1);
        
        this.selectedChange.emit(m1);
        this.cdRef.markForCheck();
