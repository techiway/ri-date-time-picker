...
export const RI_DATETIME_VALUE_ACCESSOR: any = {
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => RIDateTimeInputDirective),
    multi: true
};

export const RI_DATETIME_VALIDATORS: any = {
...
@Directive({
    selector: 'input[riDateTime]',
    exportAs: 'riDateTimeInput',
    host: {
        '(keydown)': 'handleKeydownOnHost($event)',
        '(blur)': 'handleBlurOnHost($event)',
        '(input)': 'handleInputOnHost($event)',
        '(change)': 'handleChangeOnHost($event)',
        '[attr.aria-haspopup]': 'riDateTimeInputAriaHaspopup',
        '[attr.aria-owns]': 'riDateTimeInputAriaOwns',
        '[attr.min]': 'minIso8601',
        '[attr.max]': 'maxIso8601',
        '[disabled]': 'riDateTimeInputDisabled'
    },
    providers: [
        RI_DATETIME_VALUE_ACCESSOR,
        RI_DATETIME_VALIDATORS,
    ],
})
export class RIDateTimeInputDirective<T>
    implements
        OnInit,
        AfterContentInit,
        OnDestroy,
     ...
     * */
    @Input()
    set riDateTime(value: RIDateTimeComponent<T>) {
     ...
     */
    @Input()
    set riDateTimeFilter(filter: (date: T | null) => boolean) {
    ...
    set selectMode(mode: SelectMode) {
        if (
            ...
            throw Error('RIDateTime Error: invalid selectMode value!');
        }
    ...
    /** The date-time-picker that this input is associated with. */
    public dtPicker: RIDateTimeComponent<T>;
    ...
    private parseValidator: ValidatorFn = (): ValidationErrors | null => {
        return this.lastValueValid
            ? null
            : { riDateTimeParse: { text: this.elmRef.nativeElement.value } };
    }
    ...
    private minValidator: ValidatorFn = (
        ...
        if (this.isInSingleMode) {
            ...
            return !this.min ||
                ...
                ? null
                : { riDateTimeMin: { min: this.min, actual: controlValue } };
    ...
    private maxValidator: ValidatorFn = (
        ...
        if (this.isInSingleMode) {
            ...
            return !this.max ||
                ...
                ? null
                : { riDateTimeMax: { max: this.max, actual: controlValue } };
        } else if (this.isInRangeMode && control.value) {
            const controlValueFrom = this.getValidDate(
                this.dateTimeAdapter.deserialize(control.value[0])
            );
    ...
    private requiredRangeValidator: ValidatorFn = (
        ...
        return !controlValueFrom ||
            !controlValueTo
            ? { riRequiredDateTimeRange: [controlValueFrom, controlValueTo] }
            : null;
    }
    ...
    get riDateTimeInputAriaHaspopup(): boolean {
        return true;
    }

    get riDateTimeInputAriaOwns(): string {
    ...
    get riDateTimeInputDisabled(): boolean {
    ...
    constructor(
        ...
        @Optional() @Inject(RI_DATE_TIME_FORMATS)
        private dateTimeFormats: RIDateTimeFormats ) {
        if (!this.dateTimeAdapter) {
            throw Error(
                `RIDateTimePicker: No provider found for DateTimePicker. You must import one of the following ` +
                    `modules at your application root: RINativeDateTimeModule, RIMomentDateTimeModule, or provide a ` +
        ...
        if (!this.dateTimeFormats) {
            throw Error(
                `RIDateTimePicker: No provider found for RI_DATE_TIME_FORMATS. You must import one of the following ` +
                    `modules at your application root: RINativeDateTimeModule, RIMomentDateTimeModule, or provide a ` +
                    `custom implementation.`
            );
        }
    ...
    public ngOnInit(): void {
        if (!this.dtPicker) {
            throw Error(
                `RIDateTimePicker: the picker input doesn't have any associated ri-date-time component`
            );
        }
    }
     ...
     */
    private registerDateTimePicker(picker: RIDateTimeComponent<T>) {
        if (picker) {
